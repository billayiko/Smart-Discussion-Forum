...
<flux:dropdown position="bottom" align="start">
    <button type="button" class="group flex w-full items-center rounded-lg p-1 hover:bg-zinc-800/5 dark:hover:bg-white/10" data-test="sidebar-menu-button">
        ...
        <div class="in-data-flux-sidebar-collapsed-desktop:hidden mx-2 grid flex-1 text-start text-sm leading-tight">
            <span class="truncate font-medium text-zinc-500 group-hover:text-zinc-800 dark:text-white/80 dark:group-hover:text-white">{{ $user?->name }}</span>
            <span class="truncate text-xs text-zinc-400 dark:text-zinc-500">{{ $user?->roleLabel() }}</span>
            @if($showTeam && $user?->currentTeam)
                <span class="truncate text-xs text-zinc-400 dark:text-zinc-500">{{ $user?->currentTeam->name }}</span>
            @endif
        </div>
    ...
    <flux:menu>
        <div class="flex items-center gap-2 px-1 py-1.5 text-start text-sm">
            ...
            <div class="grid flex-1 text-start text-sm leading-tight">
                ...
                <flux:text class="truncate text-zinc-500">{{ $user?->roleLabel() }}</flux:text>
            </div>
        </div>
        <flux:menu.separator />
